        ...
        try:
            ...
            for name in all:
                if name.startswith("Texturemat_TABLETOPMAT_e"):
                    ...
                    print(link_node.image.filepath)
                    ...
                    for obj in list_of_children:
                        ...
                        scene.recursive_show(obj.children)
                    ...
                    for i in cameras:
                        ...
                        bpy.data.scenes["Scene.001"].render.filepath=f"{renderdir}{name}.{scene.Leg_Type}.{i.name}"
                        if f"{name}.{scene.Leg_Type}.{i.name}.jpg" in os.listdir(renderdir):
                            self.report({'INFO'}, f"{name}.{scene.Leg_Type}.{i.name}.jpg already exists")
                            break
                        else:
            ...
            self.report({'INFO'}, "FILEPATH INCORRECT"  )
            return {'CANCELLED'}
